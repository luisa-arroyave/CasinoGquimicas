...
<div class="max-w-xl space-y-6">
    ...
    @elseif($casinos->isEmpty())
        <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 sm:p-6 text-amber-900">
            <p class="font-medium">No hay puntos de entrega disponibles para su empresa.</p>
            <p class="mt-2 text-sm">Contacte al administrador para configurar un casino o punto de entrega.</p>
        </div>
    @else
        ...
        @if($codigoQrActivo ?? null)
            <div class="rounded-xl border-2 border-emerald-200 bg-emerald-50/50 p-4 sm:p-6 shadow-sm" id="bloque-qr-activo">
                ...
                <p class="text-slate-600 text-sm mb-4">Presente este código QR en el punto de entrega para que lo validen.</p>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm">
            <p class="text-slate-600 text-sm sm:text-base mb-6">Solicite su consumo para el horario actual. Solo debe elegir el punto de entrega y, si aplica, la dirección.</p>
            ...
            <form action="{{ route('solicitar-consumo.store') }}" method="POST" class="space-y-5" id="form-solicitar">
                ...
                <div class="rounded-lg bg-slate-50 p-3 sm:p-4 space-y-3">
                    ...
                    @if($casinoPorDefecto)
                        <div>
                            <span class="text-xs font-medium text-slate-500 uppercase tracking-wide">Punto de entrega</span>
                            <p class="mt-0.5 text-base font-medium text-slate-800">{{ $casinoPorDefecto->nombre }}</p>
                            <input type="hidden" name="id_casino" value="{{ $casinoPorDefecto->id_casino }}">
                        </div>
                    @else
                        <div>
                            <label for="id_casino" class="block text-xs font-medium text-slate-500 uppercase tracking-wide mb-1">Punto de entrega</label>
                            <select name="id_casino" id="id_casino" required
                                ...
                                @foreach($casinos as $c)
                                    <option value="{{ $c->id_casino }}"
                                            ...
                                            {{ old('id_casino', $casinos->first()?->id_casino) == $c->id_casino ? 'selected' : '' }}>
                ...
                <div id="campo-direccion" class="{{ $casinoPorDefecto && strtolower(trim($casinoPorDefecto->tipo_casino ?? '')) === 'domicilio' ? '' : 'hidden' }}">
