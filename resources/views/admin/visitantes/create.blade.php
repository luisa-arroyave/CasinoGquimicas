...
@section('page-title', 'Nuevo visitante')

@section('content')
<div class="max-w-lg">
    <form action="{{ route('admin.visitantes.store') }}" method="POST" class="space-y-4 bg-white p-6 rounded-xl border border-slate-200">
        @csrf
        <div>
        ...
        <div>
            ...
            <select name="empresa_visita" id="empresa_visita" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500 min-h-[44px]">
                ...
                @foreach($empresas as $e)
                    <option value="{{ $e->nombre }}" {{ old('empresa_visita') == $e->nombre ? 'selected' : '' }}>{{ $e->nombre }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="area_visita" id="area_visita" value="{{ old('area_visita') }}" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
