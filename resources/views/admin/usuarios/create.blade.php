...
<div class="max-w-4xl">
    ...
    <form action="{{ route('admin.usuarios.store') }}" method="POST" class="space-y-4 sm:space-y-5 bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div class="rounded-lg border border-slate-200 bg-amber-50/50 px-4 py-3">
            <p class="text-sm text-slate-700 font-medium">Contraseña inicial</p>
            <p class="text-xs text-slate-600 mt-1">La contraseña inicial será el mismo número de documento. El usuario deberá cambiarla en el primer inicio de sesión.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <select name="id_tipo_usuario" id="id_tipo_usuario" required class="mt-0 block w-full border border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
                    @foreach($tipos as $t)
                        <option value="{{ $t->id_tipo_usuario }}" data-tipo-nombre="{{ $t->nombre }}" {{ old('id_tipo_usuario') == $t->id_tipo_usuario ? 'selected' : '' }}>{{ $t->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="max-w-sm">
                <select name="id_empresa_temporal" id="id_empresa_temporal" class="mt-0 block w-full border border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
                    ...
                    @foreach($empresasTemporales as $et)
                        <option value="{{ $et->id_empresa_temporal }}" {{ old('id_empresa_temporal') == $et->id_empresa_temporal ? 'selected' : '' }}>{{ $et->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="max-w-sm">
                <select name="id_casino_asignado" id="id_casino_asignado" class="mt-0 block w-full border border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
                    ...
                    @foreach($casinos as $c)
                        <option value="{{ $c->id_casino }}" {{ old('id_casino_asignado') == $c->id_casino ? 'selected' : '' }}>{{ $c->nombre }}</option>
                    @endforeach
                </select>
        ...
        {{-- Sede principal y sedes adicionales (empleados: para solicitar consumo por sede) --}}
        <div id="bloque-sede" class="hidden rounded-xl border border-slate-200 bg-slate-50/50 p-4 space-y-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mb-2">Sede donde suele estar el empleado. Los casinos que ve al solicitar consumo serán los de esta sede (y las adicionales).</p>
                <select name="id_sede_principal" id="id_sede_principal" class="mt-0 block w-full border border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500 max-w-sm">
                    ...
                    @foreach($sedes as $s)
                        <option value="{{ $s->id_sede }}" {{ old('id_sede_principal') == $s->id_sede ? 'selected' : '' }}>{{ $s->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mb-2">Marque las sedes donde también puede solicitar consumo (ej. externos en varias oficinas).</p>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto">
                    @foreach($sedes as $s)
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="sedes[]" value="{{ $s->id_sede }}" class="rounded border-slate-300 text-slate-600 focus:ring-slate-500" {{ in_array($s->id_sede, old('sedes', [])) ? 'checked' : '' }}>
    ...
    function toggle() {
        ...
        if (bloqueSede) {
            if (nombre === 'empleado') {
                ...
                bloqueSede.querySelectorAll('input[name="sedes[]"]').forEach(function(cb) { cb.checked = false; });
            }
        }
    }
