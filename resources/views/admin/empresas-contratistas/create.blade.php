...
@section('title', 'Nueva empresa contratista')
@section('page-title', 'Nueva empresa contratista')

@section('content')
<div class="max-w-lg">
    <form action="{{ route('admin.empresas-contratistas.store') }}" method="POST" class="space-y-4 bg-white p-6 rounded-xl border border-slate-200">
        @csrf
        <div>
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="activa" id="activa" value="1" {{ old('activa', true) ? 'checked' : '' }} class="rounded border-slate-300 h-4 w-4">
