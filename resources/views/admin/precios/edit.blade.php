...
@section('content')
<div class="max-w-lg">
    <form action="{{ route('admin.precios.update', $precio) }}" method="POST" class="space-y-4 bg-white p-6 rounded-xl border border-slate-200">
        @csrf
        @method('PUT')
        <div>
            ...
            <select name="id_horario" id="id_horario" required class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
                @foreach($horarios as $h)
                    <option value="{{ $h->id_horario }}" {{ old('id_horario', $precio->id_horario) == $h->id_horario ? 'selected' : '' }}>{{ $h->nombre }}</option>
                @endforeach
            </select>
            @error('id_horario')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>
        <div>
            <label for="id_casino" class="block text-sm font-medium text-slate-700">Casino (vacío = global)</label>
            <select name="id_casino" id="id_casino" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
                ...
                @foreach($casinos as $c)
                    <option value="{{ $c->id_casino }}" {{ old('id_casino', $precio->id_casino) == $c->id_casino ? 'selected' : '' }}>{{ $c->nombre }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="number" name="precio_empleado" id="precio_empleado" value="{{ old('precio_empleado', $precio->precio_empleado) }}" step="0.01" min="0" required class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
        ...
        <div>
            ...
            <input type="number" name="precio_casino" id="precio_casino" value="{{ old('precio_casino', $precio->precio_casino) }}" step="0.01" min="0" required class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
