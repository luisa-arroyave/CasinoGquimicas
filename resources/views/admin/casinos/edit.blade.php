            ...
            <div class="md:col-span-2">
                ...
                <div class="rounded-lg border border-slate-300 bg-slate-50/50 p-4 max-h-48 overflow-y-auto">
                    <div class="flex flex-wrap gap-x-6 gap-y-2">
                        @php $empresasSeleccionadas = old('empresas', $casino->empresas->pluck('id_empresa')->toArray()); @endphp
                        @foreach($empresas as $e)
                        <label class="flex items-center gap-2 cursor-pointer hover:bg-slate-100 rounded px-2 py-1 -mx-2 -my-1">
                            <input type="checkbox" name="empresas[]" value="{{ $e->id_empresa }}" {{ in_array($e->id_empresa, $empresasSeleccionadas) ? 'checked' : '' }} class="rounded border-slate-300 text-slate-800 focus:ring-slate-500">
